const getAverage = (sum, noOfElements) => sum / noOfElements;
...
const filterIncompleteProfiles = function (users) {
  return users.filter((user) => user.profileComplete === false);
};
...
const filterRecentOrders = function (orders) {
  return orders.filter((order) => order.orderDate < 23 && order);
};

// products with a price lower than the average [{name: "item1", price: 10}, {name: "item2", price: 20}, {name: "item3", price: 5}] => [{name: "item1", price: 10}, {name: "item3", price: 5}]
const filterBelowAveragePrice = function (products) {
  const sumOfPrices = products.reduce(
    (accumlator, product) => accumlator + product.price,
    0
  );
  const average = getAverage(sumOfPrices, products.length);
  return products.filter((product) => product.price < average);
};
...
const filterHighValueOrders = function (orders) {
  const sumOfOrderValue = orders.reduce(
    (accumlator, order) => accumlator + order.amount,
    0
  );
  const average = getAverage(sumOfOrderValue, orders.length);
...
const filterTopRatedBooks = function (books) {
  const sumOfRatings = books.reduce(
    (accumlator, book) => accumlator + book.rating,
    0
  );
  const average = getAverage(sumOfRatings, books.length);
...
// Input: [1, 2, 3, 4, 5], [2, 4, 6]
// Output: [1, 3, 5]
const filterByExclusion = (numbers, criteria) =>
  numbers.filter((number) => !criteria.includes(number));
...
const filterObjectsByProperties = (objects, properties) =>
  objects.filter((object) => properties[0] in object);

// Filter strings from the first array where the substring exists in the second array
// Input: ["hello", "world", "hell", "how"], ["hell", "hello"]
// Output: ["hello", "hell"]
const filterStringsBySubstringMembership = (strings, criteria) =>
  strings.filter((string) => criteria.includes(string));

// Filter numbers from the first array that fall within a range specified by a pair in the second array
// Input: [1, 2, 3, 4, 5], [[2, 4]]
// Output: [2, 3, 4]
const filterByRange = (numbers, ranges) =>
  numbers.filter((number) => number >= ranges[0][0] && number <= ranges[0][1]);

// Filter numbers from the first array that are present in the second array and are even
// Input: [1, 2, 3, 4, 5], [2, 4, 6]
// Output: [2, 4]
const filterEvenNumbersByMembership = (numbers, criteria) =>
  numbers.filter((number) => criteria.includes(number));
...
// Output: ["India", "Iran"]
const findCountriesThatExist = (countries, lookup) =>
  countries.filter((country) => lookup[country].includes("exists"));

// Find numbers that are marked as 'valid' in the lookup object.
// Input: [10, 20, 30, 40], {10: "valid", 20: "invalid", 30: "valid", 40: "valid"}
// Output: [10, 30, 40]
const findValidNumbers = (numbers, lookup) =>
  numbers.filter((number) => lookup[number] === "valid");
...
const findActiveUsers = (users, lookup) =>
  users.filter((user) => lookup[user].status === "active");
...
// Output: ["banana"]
const findStringsAboveThreshold = (strings, lookup) =>
  strings.filter((string) => lookup[string] < string.length);
...
// Output: ["Phone", "Tablet"]
const findAffordableProducts = (products, lookup) =>
  products.filter((product) => lookup[product].price < 1000);
...
// Output: ["John", "Alice"]
const findHighScoringStudents = (students, lookup) =>
  students.filter((student) => lookup[student] > 80);
...
// Output: ["John", "Bob"]
const findLongTermEmployees = (employees, lookup) =>
  employees.filter((employee) => lookup[employee].yearsAtCompany > 5);
...
// Output: ["London", "Tokyo"]
const findLargeCities = (cities, lookup) =>
  cities.filter((city) => lookup[city].population > 3000000);
...
// Output: ["item1", "item3"]
const findInStockItems = (items, lookup) =>
  items.filter((item) => lookup[item].quantity > 10);
...
const findAnimalsByHabitat = (animals, lookup, target) =>
  animals.filter((animal) => lookup[animal].habitat === target);
